  ...
  return <main className="mx-auto w-full max-w-7xl px-5 py-8"><div className="space-y-6">
    ...
    <section className="card overflow-hidden">
      <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">{entries.map((entry) => <tr key={entry.id} className={`align-top ${entry.reverses_entry_id ? "border-l-4 border-amber-400 bg-amber-50/60 hover:bg-amber-50" : "hover:bg-slate-50/70"}`}>
          ...
          <td className="max-w-56 px-5 py-4">{entry.description || <span className="text-slate-400">—</span>}{entry.reverses_entry_id && <div className="mt-2 flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-900">↶ Reversal</span><span className="text-xs text-amber-800">Reverses <span className="font-mono font-semibold">{entryNumbers.get(entry.reverses_entry_id) ?? "original entry"}</span></span></div>}</td>
